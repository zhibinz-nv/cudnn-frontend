...
from .glu_mxfp8_col_requant import Mxfp8ColRequant
...
class Sm107MegaMoEMxfp8GluKernel(Sm107Mxfp8GluFc12Kernel, KernelClass):
    ...
    col_quant_sizes_region = "rubin.glu_mxfp8.mega.col_quant_expert_token_sizes"
    ...
    def impl_desc_require(cls):
        return {
            ...
            "enable_col_quant": OptionalRequirement(bool),
            "col_quant_num_ctas": OptionalRequirement(int),
        }
    ...
    def aot_compile(self, out_path: Optional[str] = None, **_compile_kwargs):
        ...
        if self.generate_c:
            fake_arguments["fc1_c"] = fake_tensor(output_dtype, aux_shapes["fc1_c"], (1, 0), set(), 16)
        else:
        ...
        if self.enable_col_quant:
            ...
            fake_arguments["col_quant_data"] = None
            fake_arguments["col_quant_sf"] = None
    ...
    def from_kwargs(
        ...
        *,
        ...
        # -1 = let Mxfp8ColRequant derive the grid from the resident-CTA quantum.
        # A hardcoded CTA count is not generally a multiple of that quantum, which
        # leaves a fractional resident wave.
        col_quant_num_ctas: int = -1,
        ...
        impl_desc = ImplDesc(
            {
                ...
                **({"fc2_tma_stages": fc2_tma_stages} if fc2_tma_stages is not None else {}),
                # Col-quant keys present only when enabled
                **(
                    {
                        "enable_col_quant": True,
                        "col_quant_num_ctas": col_quant_num_ctas,
                    }
                    if enable_col_quant
                    else {}
                ),
    ...
    def __init__(self, problem_desc: ProblemDesc, impl_desc: ImplDesc) -> None:
        ...
        self.enable_col_quant = bool(impl_desc.get("enable_col_quant") or False)
        self._col_quant_num_ctas = int(impl_desc.get("col_quant_num_ctas") or -1)
        ...
        self.threads_per_cta = 32 * (len(self.epilogue_warp_id) + 4 + len(self.dispatch_warp_id) + num_token_back_warps)
        ...
        # Optional standalone token-axis (column) MXFP8 requantization.
        if self.enable_col_quant:
    ...
    def get_aux_output_shapes(self) -> dict:
        """Return receiver-domain auxiliary output shapes."""
        return {
            "fc1_c": (self.pool_token_capacity, self.intermediate_gateup),
            "col_quant_data": (self.pool_token_capacity, self.hidden),
            "col_quant_sf": (
    ...
    def _validate_fixed_matrix(
        ...
        if cutlass.const_expr(cute.rank(tensor.layout) != 2):
            raise ValueError("pool-domain matrix must be rank 2.")
    ...
    def _validate_fixed_vector(self, tensor: cute.Tensor, dtype, expected_size: int) -> None:
        if cutlass.const_expr(tensor.element_type is not dtype):
            raise TypeError("pool-domain vector has an unexpected element type.")
        if cutlass.const_expr(cute.rank(tensor.layout) != 1):
            raise ValueError("pool-domain vector must be rank 1.")
        if cutlass.const_expr(
            not isinstance(tensor.shape[0], int) or tensor.shape[0] != expected_size
        ):
            raise ValueError(f"pool-domain vector must have static size {expected_size}.")
        if cutlass.const_expr(tensor.stride[0] != 1):
            raise ValueError("pool-domain vector must be contiguous.")
    ...
    def _build_megamoe_device_workspace(self) -> DeviceWorkspace:
        ...
        if self.enable_col_quant:
            # Persistent per-expert token-count snapshot for the post-kernel col-quant launch.
            device_workspace.register(
                self.col_quant_sizes_region,
                cutlass.Int32,
    ...
    def token_comm_hook_kernel_tail(self, token_comm_args, *, warp_idx, lane_idx, tidx):
        ...
        """
        # Preserve the per-expert dispatch token counts before reset_tail zeros the
        # shared sizes_region.
        if cutlass.const_expr(self.enable_col_quant):
            self._snapshot_col_quant_expert_sizes(tidx)
    ...
    @cute.jit
    def _snapshot_col_quant_expert_sizes(self, tidx) -> None:
        from cutlass.cutlass_dsl import Int32, Int64
        """CTA 0 copies this rank's per-expert token counts into the persistent
        col-quant region before token_comm's tail_reset zeros ``sizes_region``."""
        ...
        if self.token_comm._linear_cta_idx == Int32(0):
            ...
            snapshot = dw.tensor(self.col_quant_sizes_region)
    ...
    def __call__(
        ...
        fc1_c: Optional[cute.Tensor],  # (pool_token_capacity, intermediate_gateup) when generate_c=True
        col_quant_data: Optional[cute.Tensor],  # dispatch-pool-strided fp8 segments
        col_quant_sf: Optional[cute.Tensor],  # flat concat_e [hidden_atom][token_atom] E8M0 bytes
        overflow_flag: cute.Tensor,  # (1,) Int32, per-rank router receive-overflow output
        ...
        if cutlass.const_expr(self.generate_c):
            if cutlass.const_expr(fc1_c is None):
                raise ValueError("generate_c=True requires a receiver-domain fc1_c tensor.")
            self._validate_fixed_matrix(
                fc1_c, cutlass.BFloat16, aux_shapes["fc1_c"]
            )
        if cutlass.const_expr(self.enable_col_quant):
        ...
        # Top-k weights were already applied before FC1 quantization, so the
        # post-kernel reduction is a plain dequantized sum.
        ...
        # Post-kernel token-axis MXFP8 requantization of the preserved dispatch-local
        # FC1 activation pool, stored by the tail snapshot of per-expert token counts.
        if cutlass.const_expr(self.enable_col_quant):
            lw = local_workspace
            data_offset = dw.offset(self.token_comm.fc1_activation_region)
            sf_offset = dw.offset(self.token_comm.fc1_activation_sf_region)
            sizes_offset = dw.offset(self.col_quant_sizes_region)
