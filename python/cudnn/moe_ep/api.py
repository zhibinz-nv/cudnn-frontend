...
def _validate_training_assert_capability(config: ForwardConfig) -> None:
    ...
    if not callable(getattr(torch, "_assert_async", None)):
        raise RuntimeError(
            "drop_on_overflow=False training plan requires callable "
    ...
    if backend != dist.Backend.NCCL and str(backend).lower() != "nccl":
        raise NotImplementedError(
            "drop_on_overflow=False EP2+ training plan requires an NCCL "
...
class MoeEp:
    ...
    :meth:`prepare_training`; the returned slotless plan borrows caller-owned
    contexts and outputs for ordinary or capturable ``forward``/``backward``.
    ...
    def __init__(
        ...
        *,
        ...
        self._training_plan: MoeEpTrainingPlan | None = None
        self._closed = False
    ...
    def __call__(
        ...
        Training callers must use :meth:`prepare_training` and its caller-owned
        context ABI.
        """
    ...
    def prepare_training(
        ...
        *,
        ...
        """Bind MXFP8 weights and prepare a slotless training execution plan.

        This collective preparation must run on every EP rank before CUDA
        Graph capture. The returned plan owns only mutable per-stream lanes;
        callers allocate microbatch contexts from ``buffer_specs``. Closing
        the operator also closes the plan. A closed plan cannot be replaced on this
        ...
        with self._lifecycle_lock:
            ...
            if self._training_plan is not None:
                if not self._training_plan.closed:
                    raise RuntimeError("MoeEp training plan already exists")
                raise RuntimeError(
                    "MoeEp training plan was closed; create a new MoeEp "
                    "instance before preparing replacement weights"
                )
            ...
            owner = self._forward_backend.prepare_training(
                weights,
    ...
    def training_buffer_specs(self) -> MoeEpTrainingBufferSpecs:
        """Return the prepared plan's caller-owned fixed-capacity buffer ABI."""

        with self._lifecycle_lock:
            if self._closed:
                raise RuntimeError("MoeEp is closed")
            if self._training_plan is None or self._training_plan.closed:
                raise RuntimeError(
                    "prepare_training must be called before training_buffer_specs"
                )
            return self._training_plan.buffer_specs
